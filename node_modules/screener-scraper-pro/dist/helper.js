export function getParsedDataFromHtml(parsedHtml) {
    return {
...
export function processAnalysis(analysis) {
    const pros = analysis
        .find('.pros ul li')
...
// }
export function processAbsoluteTable(section) {
    const table = section.find('table.data-table');
    // Extract column headers from thead
    const headers = ['']; // First column is row label
    ...
    table.find('tbody tr').each((_, row) => {
        ...
        const button = firstCell?.children.find((child) => child.type === 'tag' && child.name === 'button');
        if (button) {
...
export function processDocuments(section) {
    ...
    const findChild = (el, tagName) => {
        return el.children.find((child) => child.type === 'tag' && child.name === tagName);
    };
    // Helper to find all child elements by tag name
    const findChildren = (el, tagName) => {
        return el.children.filter((child) => child.type === 'tag' && child.name === tagName);
    };
...
export function processPnlToFindCAGRs(section) {
    const cagrs = {};
    section.find('table.ranges-table').each((_, table) => {
        // Get table title from th
        let title = '';
        const thRow = table.children.find((child) => child.type === 'tag' && (child.name === 'tbody' || child.name === 'thead'));
        if (thRow) {
            const firstTr = thRow.children.find((child) => child.type === 'tag' && child.name === 'tr');
            if (firstTr) {
                const th = firstTr.children.find((child) => child.type === 'tag' && child.name === 'th');
        ...
        const tbody = table.children.find((child) => child.type === 'tag' && child.name === 'tbody');
        if (tbody) {
            const rows = tbody.children.filter((child) => child.type === 'tag' && child.name === 'tr');
